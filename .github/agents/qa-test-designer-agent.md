...
El prompt incluye un campo *MODO* en la sección ENFOQUE ESPECÍFICO que determina qué artefactos generar:

- *MODO: Solo casos automatizables* → Genera ÚNICAMENTE casos_automatizables.md y automation_v1.spec.ts. NO generar casos_manuales.md.
- *MODO: Solo casos manuales* → Genera ÚNICAMENTE casos_manuales.md. NO generar casos_automatizables.md ni automation_v1.spec.ts.
- *MODO: Ambos — priorizando automatizados* → Genera los 3 artefactos. El mayor volumen de casos debe ser automatizable; manuales solo lo estrictamente necesario.

Si no se indica un MODO explícito, aplica "Ambos — priorizando automatizados" por defecto.
...
Ejecuta *directamente y sin pedir confirmación* los comandos de shell para crear:

1. *Detectar carpeta destino:* La carpeta ya existe en qa-portal/src/assets/repo-files/[Sprint]/. Busca la carpeta cuyo nombre *empiece con el mismo código EN/HU* del prompt (p.ej. si el prompt dice EN31295, usa la carpeta que empiece con EN31295). *NO crear carpeta nueva*; escribe los artefactos dentro de la carpeta existente.
...
## DATOS DE PRUEBA (OBLIGATORIO en spec.ts)

En automation_v1.spec.ts, cada test debe incluir un objeto testData con datos sugeridos:

typescript
const testData = {
  valid: { /* datos para happy path */ },
  invalid: { /* datos que disparan errores */ },
  edge: { /* datos límite/borde */ }
};
...
Usa estrictamente para los .md:
 ...
 CP-001: [Titulo descriptivo del caso]   ← automatizables: CP-001, CP-002, CP-003...
 CP-M01: [Titulo descriptivo del caso]  ← manuales: CP-M01, CP-M02, CP-M03...
...
- Incluir objeto testData por test con datos válidos, inválidos y borde.
- Adaptar selectores según tipo de app: Web (DOM), API (fetch/request), Móvil (locators nativos).
...
- *Nomenclatura OBLIGATORIA*: casos automatizables deben nombrarse CP-001, CP-002, CP-003... y casos manuales CP-M01, CP-M02, CP-M03... Sin excepción.